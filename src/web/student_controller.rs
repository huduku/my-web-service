...
use crate::domain::{
    cmd::student_cmd::{StudentCreate, StudentQuery, StudentUpdate},
    core::IdOper
};
...
pub(crate) async fn get_student_handler(
    ...
    ValidQuery(stu, ..): ValidQuery<StudentPO, IdOper<i64>>,
) -> impl IntoResponse {
...
pub(crate) async fn post_student_handler_form(
    // State(srb): State<Arc<AppState>>,
    ValidForm(stu, ..): ValidForm<StudentPO, IdOper<i64>>,
) -> impl IntoResponse {
    JsonRes(get_student(pool!(), stu.id.unwrap()).await)
}
...
pub(crate) async fn create_student_handler(
    // State(srb): State<Arc<AppState>>,
    ValidJson(student, ..): ValidJson<StudentPO, StudentCreate>,
) -> impl IntoResponse {
    JsonRes(create_student(pool!(), student).await)
}

pub(crate) async fn update_student_handler(
    // State(srb): State<Arc<AppState>>,
    ValidJson(student,..): ValidJson<StudentPO, StudentUpdate>,
) -> impl IntoResponse {
    JsonRes(update_student(pool!(), student).await)
}
...
pub async fn list_students_handler(
    // State(srb): State<Arc<AppState>>,
    ValidJson(req, ..): ValidJson<PageReq<StudentPO>, StudentQuery>,
) -> impl IntoResponse {
    let rb = pool!();
    match list_students(rb, req).await {
